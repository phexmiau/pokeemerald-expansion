...
static const u32 sTitleScreenRayquazaGfx[] = INCBIN_U32("graphics/title_screen/trans_sky_tile.4bpp.lz");
static const u32 sTitleScreenRayquazaTilemap[] = INCBIN_U32("graphics/title_screen/trans_sky_tile.bin.lz");
        ...
        if (sprite->sMode != SHINE_MODE_SINGLE_NO_BG_COLOR)
        {
            ...
            if (sprite->x == DISPLAY_WIDTH / 2 + (3 * SHINE_SPEED)
             ...
             || sprite->x == DISPLAY_WIDTH / 2 + (6 * SHINE_SPEED))
                gPlttBufferFaded[0] = RGB(31, 12, 31);
            else
